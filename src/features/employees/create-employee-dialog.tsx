...
export function CreateEmployeeDialog() {
  ...
  const reset = () =>
    setForm({ email: "", password: "", name: "", phone: "", department: "", hourlyRate: "", payCalculationMode: "HOURLY" });

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.email || !form.password || !form.hourlyRate) {
      toast.error("Email, password and hourly rate are required");
      return;
    }
    ...
    createEmployee.mutate(
      ...
      {
        onSuccess: () => {
          toast.success("Employee added", { description: `${form.name || form.email} was added.` });
          setOpen(false);
          reset();
        },
        onError: (error) => toast.error("Couldn't add employee", { description: getApiErrorMessage(error) }),
      },
    );
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Create an account and pay profile for this employee.</DialogDescription>
        </DialogHeader>
